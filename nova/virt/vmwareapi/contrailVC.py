...
class VCPVlans(object):
    ...
    def _init_pvlans(self, session):
	    ...
	    vlan = cfg_ret.defaultPortConfig.vlan
            if hasattr(vlan, "pvlanId"):
                self._vlan_id_bits[vlan.pvlanId] = 1
...
class ContrailVCDriver(VMwareVCDriver):
    ...
    def destroy(self, instance, network_info, block_device_info=None,
        ...
        for vif in network_info['info_cache']['network_info']:
            #import pdb; pdb.set_trace()
            network_uuid = vif['network']['bridge']
            ...
            vms_mor = vms_ret.ManagedObjectReference
            if not vms_mor:
                self.remove_dvport_group(network_uuid)
